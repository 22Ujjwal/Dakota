...
class EnhancedBrowserAgent {
    constructor(options = {}) {
        ...
        this.modelConfig = {
            textModel: 'gpt-4o',  // Latest GPT-4 model
            visionModel: 'gpt-4o',  // GPT-4o has vision capabilities
            fallbackTextModel: 'gpt-4-turbo',
            fallbackVisionModel: 'gpt-4-vision-preview'
        };
    ...
    async decide(observation) {
        ...
        try {
            ...
            // Use latest GPT model with screenshot analysis for better decision making
            const decisionText = await this.createOpenAIRequest(
                prompt, 
    ...
    async evaluate(actionResult) {
        ...
        try {
            ...
            // Use latest GPT model for evaluation
            const evaluationText = await this.createOpenAIRequest(
                evaluationPrompt,
                currentObservation.screenshot,
                { maxTokens: 800 }
            );
            
            const aiEvaluation = this.parseEvaluation(evaluationText);
...
`;
    ...
    /**
     * Create OpenAI request with model fallbacks and future compatibility
     */
    async createOpenAIRequest(prompt, screenshot = null, options = {}) {
        const maxTokens = options.maxTokens || 1000;
        
        try {
            // Prepare messages for current API format
            let messages;
            let model;
            
            if (screenshot) {
                model = this.modelConfig.visionModel;
                messages = [
                    {
                        role: "user",
                        content: [
            ...
            // Fallback to older models if newer ones fail
            this.log(`Primary model failed, trying fallback: ${error.message}`, 'warning');
            
            const fallbackModel = screenshot ? this.modelConfig.fallbackVisionModel : this.modelConfig.fallbackTextModel;
