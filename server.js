...
// Analyze image with OpenAI GPT-5-mini (with rate limiting)
async function analyzeImage(imageData) {
  const apiCall = async () => {
    try {
      ...
      console.error('OpenAI GPT-5 Vision API error:', error);
      throw error;
    }
  };
...
// Generate automation script with OpenAI GPT-5-mini (with rate limiting)
async function generateAutomationScript(task, context = '') {
  const apiCall = async () => {
    try {
      ...
      console.error('OpenAI GPT-4 script generation error:', error);
      throw error;
    }
  };
...
console.log('Starting Dakota AI Assistant...');
console.log('Browser initialization will happen on first automation request.');

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => {
  console.log(`🚀 Dakota AI Assistant running on port ${PORT}`);
  console.log(`📱 Open http://localhost:${PORT} to access Dakota`);
